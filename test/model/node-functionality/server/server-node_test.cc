#include "model/node-functionality/server/server-node.h"
...
TEST_F(ServerNodeTest, GetUuidTest) {
  EXPECT_EQ(server_one->GetUuid(), uuid_one);
}

TEST_F(ServerNodeTest, GetUuidDoubleTest) {
  EXPECT_EQ(server_one->GetUuid(), uuid_one);
  EXPECT_EQ(server_two->GetUuid(), uuid_two);
}

TEST_F(ServerNodeTest, GetUuidConstant) {
  std::string uuid = "mitch";
  ServerNode tmp_server(uuid, name_one, owner_two);
...
TEST_F(ServerNodeTest, GetUuidMany) {
  ...
  for (int index = 0; index < 20; ++index) {
    ...
    server_array[index] = new ServerNode(uuid, name_one, owner_one);
  }

  for (int index = 0; index < 20; ++index) {
    EXPECT_EQ(server_array[index]->GetUuid(), std::to_string(index));
  }
}
...
TEST_F(ServerNodeTest, GetUuidEqual) {
  ServerNode *tmp_server = new ServerNode(uuid_one, name_one, owner_one);
...
TEST_F(ServerNodeTest, GetNameConstant) {
  ...
  ServerNode tmp_server(uuid_one, name, owner_two);
...
TEST_F(ServerNodeTest, GetNameMany) {
  ...
  for (int index = 0; index < 20; ++index) {
    ...
    server_array[index] = new ServerNode(uuid_one, name, owner_one);
  }
...
TEST_F(ServerNodeTest, GetOwnerConstant) {
  std::string owner = "mitch";
  ServerNode tmp_server(uuid_one, name_one, owner);
...
TEST_F(ServerNodeTest, GetOwnerMany) {
  ...
  for (int index = 0; index < 20; ++index) {
    ...
    server_array[index] = new ServerNode(uuid_one, name_one, owner);
  }
